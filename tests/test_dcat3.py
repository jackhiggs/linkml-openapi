...
class TestDiscriminatorPlacement:
    ...
    *only* at the join points: slot ranges (when composition reaches a
    ...
    def test_legacy_type_field_pins_per_class_constant(
        self, schemas, concrete, legacy_value
    ):
...
class TestPolymorphicInheritance:
    ...
    def test_polymorphic_class_extends_parent_via_allof(
        self, schemas, concrete, parent
    ):
...
class TestAgentClass:
    ...
    @pytest.mark.parametrize(
        "agent_slot", ["publisher", "creator", "contributor", "contactPoint"]
    )
